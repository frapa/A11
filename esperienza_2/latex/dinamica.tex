...
\item{Al fine di calcolare $\delta y_{tot}$, dobbiamo fare una stima preliminare del parametro B. Dal momento che dal grafico è impossibile individuare le rette di massima e minima pendenza abbiamo deciso di ricavare il parmetro B valutando il coefficiente angolare della retta utilizzando il classico rapporto tra due punti del grafico, ovvero:
    ...
    volte minore rispetto all'incertezza sul periodo quadrato ($\delta y_i$), e quindi poichè nella
    formula
...
%Dal momento che il chi quadro osservato, pur essendo compatibile con la predizione teorica, risulta essere molto maggiore del chi quadro teorico, abbiamo deciso di aggiustare comunque il $\chi^2$ per ottenere una migliore stima delle inceretezze sul periodo,
%che probabilmente sono state sottostimate.
%
%In questo caso le incertezze $\delta y_i^2$ sono diverse, per cui le moltiplichiamo tutte per un fattore $h$.
%Imponiamo:
%
%\begin{equation*}
%    \sum_{i=1}^{N} \frac{(y_i - A - Bx_i)^2}{h(\delta y_i)^2} \,=\, \frac{\chi_{oss}^2}{h} \,=\, \chi_{teo}^2 
%\end{equation*}
%%
%quindi sfruttando questa relazione vogliamo trovare $h$, che ci restituisce una nuova incertezza, grazie alla quale i due chi
%risulteranno uguali. Perciò otteniamo quanto segue:
%
%\begin{equation*}
%    h \,=\, \frac{\chi_{oss}^2}{\chi_{teo}^2} = 1.75
%\end{equation*}
%%
%Pertanto le incertezze $\delta y_i$ sono corrette di un fattore:
%
%\begin{equation*}
%    \sqrt{h} = 1.32
%\end{equation*}
%%
%che è, a nostro avviso, accettabile poiché non si è tenuto conto di errori sistematici nella misura del per
...
L'esperimento è andato a buon fine e le constanti elastiche risultano tutte combatibili tra di loro.
