...
\begin{itemize}
	\item{primo allungamento $x_{all_1}$ = $() cm$}
	\item{secondo allungamento $x_{all_2}$ = $() cm$}
	\item{terzo allungamento $x_{all_3}$ = $() cm$}
	\item{quarto allungamento $x_{all_4}$ = $() cm$}
	\item{quinto allungamento $x_{all_5}$ = $() cm$}
\end{itemize}
...
\begin{equation*}
	m^*[x_{all_i}] = \frac{1}{4} \sum_{i=2}^{5} (x_{all_i}) =
\end{equation*}
e l'incertezza su questo valore è il seguente:

\begin{equation}
	\sigma^*[m^*[x_{all_i}]] = \sqrt{\frac{1}{4} \sum_{i=2}^{5} (x_{all_i} - m^*[x_{all_i}])^2} = 
\end{equation}
quindi possiamo ricavare R e posto fattore di copertura ... calcoli e ...